...
class CheckpointManager:
    ...
    def consolidate_features(self, output_path: Optional[Path] = None) -> Dict[str, Any]:
        ...
        Returns:
            Dict with combined features in lapeigvals format
        """
        ...
        for path in feature_paths:
            try:
                ...
                consolidated["sample_ids"].append(data.get("sample_id", path.stem))
                consolidated["metadata"].append(data.get("metadata", {}))
                
                for key in ["attn_diags", "laplacian_diags", "hidden_states", 
                           "token_probs", "full_attention"]:
                    if key in features and features[key] is not None:
                        consolidated[key].append(features[key])
            except Exception as e:
