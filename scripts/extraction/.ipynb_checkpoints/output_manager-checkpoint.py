...
def finalize_outputs(
    ...
    Args:
        ...
        samples: 样本列表
        cfg: 配置对象（可选）
        num_workers: 并行加载的worker数量（默认8，适合2×H100）
    """
    ...
    # 执行合并（使用优化的单次遍历+多线程）
    logger.info(f"Consolidating features (single-pass with {num_workers} workers)...")
    
    features_dir.mkdir(exist_ok=True)
    
    consolidated = checkpoint_manager.consolidate_features_streaming(
        features_dir,
