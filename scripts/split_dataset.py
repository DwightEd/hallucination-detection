...
2. 加载数据集（已按model和task_type过滤）
3. 检查是否已有split字段
4. 如果没有split字段，进行分割
5. 保存train.json和test.json
...
注意：
- RAGTruth等已有split字段的数据集会自动跳过分割
- 只有没有split字段的数据集才会进行分割
"""
...
def save_splits(
    ...
    test_data: List[Dict],
    output_dir: Path,
    original_files: Optional[List[Path]] = None,
    ...
    Args:
        ...
        test_data: 测试数据
        output_dir: 输出目录
        original_files: 原始数据文件列表（用于删除）
        delete_originals: 是否删除原始文件
    """
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    
    # 保存完整的数据（不转换为 {id: item} 格式，保留所有字段）
    ...
    # 删除原始文件
    if delete_originals and original_files:
        for original_file in original_files:
            if original_file.exists():
                try:
                    ...
                    logger.warning(f"Failed to delete {original_file}: {e}")
...
def main():
    ...
    for ds_cfg in datasets_config:
        ...
        original_files = []
        if dataset_path.is_file():
            original_files = [dataset_path]
        else:
            # 检查常见的数据文件
            for filename in ['response.jsonl', 'data.jsonl', 'data.json', 'samples.jsonl', 'samples.json']:
                filepath = dataset_path / filename
                if filepath.exists():
                    original_files.append(filepath)
        
        # 是否删除原始文件
        delete_originals = split_config.get('delete_originals', False)
        ...
        save_splits(
            ...
            test_data, 
            output_dir,
            original_files=original_files if delete_originals else None,
