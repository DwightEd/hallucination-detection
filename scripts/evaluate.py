...
def evaluate_single_level(
    ...
    if not model_path.exists():
        logger.warning(f"Model not found for level '{level}': {model_path}")
        logger.warning(f"Skipping {level}-level evaluation.")
        return False, {}
    ...
    logger.info(f"[{level}] Results saved to {eval_results_path}")
...
def main(cfg: DictConfig) -> None:
    ...
    当 level="both" 时，分别评估 sample 和 token 两个级别的模型。
    """
    ...
    logger.info(f"Level config: {config_level}")
    logger.info("=" * 60)

    # 确定需要评估的级别列表
    if config_level == "both":
        levels_to_eval = ["sample", "token"]
        logger.info(f"Level config is 'both' → will evaluate both sample and token levels")
    else:
        levels_to_eval = [config_level]
    ...
    # 加载特征时，如果要评估 token 级别，需要加载 token labels
    load_token_labels = "token" in levels_to_eval
    ...
    for level in levels_to_eval:
        success, results = evaluate_single_level(
            cfg=cfg,
            pm=pm,
    ...
    for level, results in all_results.items():
        ...
        aupr = test_metrics.get('aupr', 0)
        f1 = test_metrics.get('f1', 0)
        logger.info(f"  [{level}] Test AUROC: {auroc:.4f}, AUPR: {aupr:.4f}, F1: {f1:.4f}")
    logger.info("=" * 60)
