...
Uses various entropy measures to detect hallucinations:
- Token probability entropy: Uncertainty in token predictions
- Attention entropy: How spread out attention is
- Hidden state entropy: Variation in representations
...
@METHODS.register("entropy", aliases=["token_entropy"])
class EntropyMethod(BaseMethod):
    ...
    def extract_method_features(self, features: ExtractedFeatures) -> np.ndarray:
        ...
        if len(all_features) == 0:
            raise ValueError("Entropy method requires token_entropy or attn_entropy")
